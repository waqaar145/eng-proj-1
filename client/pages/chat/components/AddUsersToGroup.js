...
const AddUsersTopGroup = ({ groupId, showChatList }) => {
  ...
  const handleAdd = async (userObj, callback) => {
    ...
    try {
      ...
      dispatch({
        type: chatActionTypes.UPDATE_MEMBERS_COUNT_OF_CURRENT_GROUP,
        data: data.new ? 1 : -1,
      });
    } catch (error) {
      console.log(error);
  ...
  const { toggle: handleLeaveGroupToggle, show: showLeaveGroupModal } =
    useModal();
  ...
  const { toggle: handleRemoveUserToggle, show: showRemoveUserModal } =
    useModal();

  const handleRemoveUserFromGroup = (user) => {
    setCurrentUserToRemoveObj({
      id: user.id,
  ...
  const { toggle: handleMakeAdminToggle, show: showMakeAdminModal } =
    useModal();

  const handleMakeAdminOfGroup = (user) => {
    setCurrentUserToBeAdminObj({
      id: user.id,
      name: `${user.firstName} ${user.lastName}`,
      admin: user.admin,
    });
  ...
  const handleMakeAdmin = async (user, callback) => {
    try {
      ...
      } = await chatService.makeAdmin({ groupId, userId: user.id });
      setUsers(
        users.map((user) => {
          if (user.id === userId) {
            return {
    ...
    <>
          ...
          <div className={styles.inputBox}>
            <SimpleInput
              ...
              label={`Search usernames ${
                activeTab === 1 && currentSelectedGroup.admin === 1
                  ? "to add"
                  : ""
              }`}
                ...
                <li>
                  <a
                    className={`${activeTab === 2 ? styles.active : ""} ${
                      currentSelectedGroup.members === 0 ? styles.disabled : ""
                    }`}
                    onClick={() => setActiveTab(2)}
                  >
                    Existing Users ({currentSelectedGroup.members})
                  </a>
                </li>
              </ul>
            </div>
            <div className={styles.leaveGroupAction}></div>
          </div>
        )}
      </div>
      <div className={styles.userList} ref={userListRef}>
        ...
        {users.length > 0 && !loading && (
          <>
            <div className={styles.separator}>
              Results ({activeTab === 1 ? totalResults : filteredUsers.length})
            </div>
          </>
        )}
        {!loading && (
                        ...
                        <>
                          ...
                          {user.added && (
                            <SimpleButton
                              ...
                              size="sm"
                              buttonStyle={`${
                                activeTab === 1 ? "primeButton" : "dangerButton"
                              }`}
      ...
      <ConfirmModal
        ...
        handleDelete={() =>
          handleAdd(currentUserToRemoveObj, hideRemoveUserModal)
        }
      ...
      <ConfirmModal
        id={groupId}
        titleText={`${
          currentUserToBeAdminObj.admin === 1
            ? `Remove ${currentUserToBeAdminObj.name} from admin of #${currentSelectedGroup.groupName} group?`
            : `Make ${currentUserToBeAdminObj.name} admin of #${currentSelectedGroup.groupName} group?`
        }`}
        bodyText="Are you sure? This can't be undone"
        handleDelete={() => {
          handleMakeAdmin(currentUserToBeAdminObj, hideMakeAdminModal);
        }}
