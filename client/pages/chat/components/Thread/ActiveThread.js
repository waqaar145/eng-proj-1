...
const ActiveThread = ({ currentActiveThread, addNewReplySocketEmitter, updateReplySocketEmitter }) => {
  ...
  let parentMessage = convertMessagesArrayToObjectForm({
    [message.id]: message,
  });
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("resize", debounce(handleResizeThread, 100));
  }, []);
  ...
  const getThreadReplies = async (id) => {
    try {
      ...
      let {
        data: { data },
      } = await chatService.getThreadReplies(
        { groupId, messageId: id },
        paramsObj
      );
      await dispatch({
        type: chatActionTypes.THREAD_REPLIES,
        data: { ...data, currentPage, messageId: id },
      });
      scrollToBottom();
    } catch (error) {
      console.log(error);
    ...
    <div className={styles.threadWrapper}>
      ...
      <div className={styles.container} ref={chatContentBodyRef}>
        ...
        <div className={styles.separator}>
          Total Replies (
          {threadReplies ? Object.keys(threadReplies).length : null})
        </div>
      ...
      <div className={styles.threadTextArea} ref={textAreaRef}>
        <MyEditor
          ...
          placeholder={`Reply in Thread ${
            currentSelectedGroup.groupType !== "private"
              ? `of #${currentSelectedGroup.groupName}`
              : ""
          }`}
        />
      </div>
