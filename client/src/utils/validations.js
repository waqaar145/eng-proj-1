...
export const stringRangeValidation = (key, value, required, min = 1, max = 200) => {
  if (required) {
    if (!key || !value) {
  ...
  if (value.length < min || value.length > max) {
    return `${capitalizeFirstLetter(key)} must be${min < 2 ? '': " between "+ min + " and"} ${min < 2 ? 'maximum '+ max  : max} characters long`;
  }
...
export const isAlpha = (key, value, required, min=1, max=50) => {
  ...
  if (value.length < min || value.length > max) {
    return `${capitalizeFirstLetter(key)} must be between ${min} and ${max} characters long`;
  }
...
export const isAlphanumeric = (key, value, required, min=1, max=50) => {
  ...
  if (value) {
    if (!value.match(alphaNumeric)) {
      return `${capitalizeFirstLetter(key)} must be alphanumeric and underscores only`;
    }
  }
